...
const MapDashboard = () => {
      ...
      `;
          ...
          <div class="min-w-[150px]">
            <div class="flex items-center justify-between gap-2 mb-1">
              ...
              <span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-slate-800 border border-slate-700 ${r.severity_level === 'high' ? 'text-red-400 border-red-900/50' :
                r.severity_level === 'medium' ? 'text-orange-400 border-orange-900/50' : 'text-emerald-400 border-emerald-900/50'}">${r.severity_level}</span>
            </div>
    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                const bestMatch = data.find((d) => d.class === 'building' || d.class === 'amenity' || d.type === 'building') || data[0];
    ...
    return (<div className="relative w-full h-[62vh] md:h-[68vh] min-h-[480px] rounded-xl overflow-hidden shadow-2xl border border-slate-700 bg-slate-900">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1000] w-full max-w-[320px] px-4">
        {selectionMode ? (<div className="w-full bg-blue-600/90 text-white px-6 py-2.5 rounded-full font-bold shadow-2xl backdrop-blur-md text-center text-xs border border-white/20 whitespace-nowrap">
            📍 Click on the map to select location
          </div>) : (<form onSubmit={handleSearch} className="relative group hidden md:block">
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search local streets/landmarks..." className="w-full glass-panel bg-slate-900/60 text-white pl-4 pr-12 py-2.5 rounded-full text-xs outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all border-white/10 group-hover:border-white/20 shadow-2xl"/>
            <button type="submit" disabled={isSearching} className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center rounded-full bg-indigo-600 hover:bg-indigo-500 text-white transition-colors disabled:opacity-50">
      ...
      <div className="absolute top-4 right-4 z-[1000] flex flex-col gap-4 w-[220px]">
        ...
        {/* Action Buttons */}
        <div className="flex flex-col gap-2">
          <button onClick={() => setSelectionMode(selectionMode === 'report' ? null : 'report')} className={`w-full ${selectionMode === 'report' ? 'bg-orange-200 shadow-orange-300/40 text-orange-800 border-orange-300/50' : 'bg-orange-100 hover:bg-orange-200 text-orange-700 border-orange-300/50'} px-4 py-2.5 rounded-lg text-xs font-extrabold transition-all duration-200 hover:scale-[1.02] active:scale-95 shadow-lg backdrop-blur-md flex items-center justify-center gap-2 border`}>
            {selectionMode === 'report' ? 'Cancel Selection' : 'Report Incident'}
          </button>
          <button onClick={() => setSelectionMode(selectionMode === 'rating' ? null : 'rating')} className={`w-full ${selectionMode === 'rating' ? 'bg-blue-200 shadow-blue-300/40 text-blue-900 border-blue-300/50' : 'bg-blue-100 hover:bg-blue-200 text-blue-800 border-blue-300/50'} px-4 py-2.5 rounded-lg text-xs font-extrabold transition-all duration-200 hover:scale-[1.02] active:scale-95 shadow-lg backdrop-blur-md flex items-center justify-center gap-2 border`}>
      ...
      {showReportForm && selectedLocation && (<div className="absolute inset-0 z-[2000] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">

          <div className="max-w-md w-full">
            <ReportForm location={{ lat: selectedLocation.lat, lng: selectedLocation.lng }} onSuccess={() => {
      ...
      {showRatingForm && selectedLocation && (<div className="absolute inset-0 z-[2000] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
