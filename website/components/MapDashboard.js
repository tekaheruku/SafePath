...
import { Calendar, FilterX, AlertCircle, Navigation } from 'lucide-react';
...
function routePolylineColor(safetyScore) {
    if (safetyScore >= 4.0)
        return '#22c55e';
    if (safetyScore >= 2.5)
        return '#f59e0b';
...
const MapDashboard = () => {
    ...
    const incidentsHeatLayerRef = useRef(null);
    const ratingsHeatLayerRef = useRef(null);
    const markersRef = useRef([]);
    const ratingMarkersRef = useRef([]);
    // Route polylines
    const routePolylinesRef = useRef([]);
    const routeMarkersRef = useRef([]); // start/end pins
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const [stats, setStats] = useState({ reports: 0, heatmapPoints: 0 });
    const { lat: storeLat, lng: storeLng, zoom: storeZoom, setView, showIncidentsHeat, showRatingsHeat, setIncidentsHeat, setRatingsHeat } = useMapStore();
    // Directions store
    const { isOpen: directionsOpen, setOpen: setDirectionsOpen, selectionTarget: directionsSelectionTarget, setSelectionTarget: setDirectionsSelectionTarget, setStartPoint: setDirectionsStart, setEndPoint: setDirectionsEnd, routes: directionRoutes, selectedRouteIndex, } = useDirectionsStore();
    ...
    const [selectedRatingId, setSelectedRatingId] = useState(searchParams.get('ratingId'));
    const [incidentHeatPoints, setIncidentHeatPoints] = useState([]);
    const [ratingHeatPoints, setRatingHeatPoints] = useState([]);
    const [showReportForm, setShowReportForm] = useState(false);
    const [showRatingForm, setShowRatingForm] = useState(false);
    const [selectedLocation, setSelectedLocation] = useState(null);
    const [selectionMode, setSelectionMode] = useState(null);
    const [selectedIncidentTypeId, setSelectedIncidentTypeId] = useState(null);
    ...
    const directionsSelectionTargetRef = useRef(null);
    useEffect(() => {
    ...
    const updateMarkers = (reports) => {
        ...
        if (showIncidentsHeatRef.current && !selectedReportId)
            return;
        reports.forEach(r => {
            // If heatmap is on, only show the selected report
            if (showIncidentsHeatRef.current && r.id !== selectedReportId)
                return;
      ...
      `;
          ...
          <div class="min-w-[150px]">
            <div class="flex items-center justify-between gap-2 mb-1">
              <strong class="text-indigo-400 font-bold capitalize text-sm flex items-center gap-1">
                <span class="text-lg leading-none">${r.incident_type_icon || ''}</span>
    ...
    const updateIncidentsHeatmap = (points) => {
        if (!mapRef.current || !incidentsHeatLayerRef.current)
            return;
        ...
        if (shouldShow) {
            ...
            layer._latlngs = points.map((p) => [p.latitude, p.longitude, p.intensity]);
    ...
    const updateRatingMarkers = (ratings) => {
        if (!mapRef.current)
            return;
        ratingMarkersRef.current.forEach(m => m.remove());
        ratingMarkersRef.current = [];
        // Hide pins if heatmap is toggled on (Convert to Heatmap feature)
        if (showRatingsHeatRef.current && !selectedRatingId)
            return;
        ratings.forEach(r => {
            if (showRatingsHeatRef.current && r.id !== selectedRatingId)
                return;
            const canDelete = user && (user.id === r.user_id || ['admin', 'superadmin', 'lgu_admin'].includes(user.role));
            const deleteHtml = canDelete
    ...
    const updateRatingsHeatmap = (points) => {
        if (!mapRef.current || !ratingsHeatLayerRef.current)
            return;
    ...
    useEffect(() => {
        if (!mapContainerRef.current || mapRef.current)
            return;
        const IBA_BOUNDS = L.latLngBounds([15.30, 119.92], // SW
        [15.41, 120.18] // NE — covers Santa Barbara far east
        );
        const initialLat = searchParams.get('lat') ? parseFloat(searchParams.get('lat')) : storeLat;
        const initialLng = searchParams.get('lng') ? parseFloat(searchParams.get('lng')) : storeLng;
        const initialZoom = searchParams.get('zoom') ? parseInt(searchParams.get('zoom')) : storeZoom;
        ...
        const incidentHeat = L.heatLayer([], {
        ...
        const t1 = setTimeout(() => { if (mapRef.current)
            map.invalidateSize(); }, 100);
        const t2 = setTimeout(() => { if (mapRef.current)
            map.invalidateSize(); }, 400);
        const t3 = setTimeout(() => { if (mapRef.current)
            map.invalidateSize({ pan: false }); }, 800);
    ...
    useEffect(() => {
        ...
        window.voteReport = async (id, type) => {
            if (!token) {
        ...
        return () => {
            ...
            delete window.deleteRating;
            delete window.voteReport;
        };
    }, [token]);
    ...
    const drawRoutesOnMap = useCallback((routes, selectedIdx) => {
        if (!mapRef.current)
            return;
        // Clear old polylines
        ...
        if (selected && selected.geometry.length >= 2) {
            ...
            const allLatLngs = selected.geometry.map(([lng, lat]) => [lat, lng]);
    ...
    return (<div className="relative w-full h-[68vh] md:h-[72vh] min-h-[520px] rounded-xl overflow-hidden shadow-2xl border border-slate-700 bg-theme-panel">
      ...
      <SearchBar onSuggestionSelect={handleSuggestionSelect} onSearch={handleSearch} isSearching={isSearching} selectionMode={selectionMode} boundaryPolygon={IBA_POLYGON}/>
        ...
        <div className="glass-panel p-4 rounded-xl text-theme-fg shadow-xl min-w-[180px]">
          <h3 className="font-extrabold text-base mb-1 bg-gradient-to-r from-blue-500 to-emerald-500 bg-clip-text text-transparent text-outline">SafePath Iba</h3>
            <div className="flex flex-col gap-1 mt-2">
             ...
             {!loading && stats.reports === 0 && dateRange.from && (<div className="flex items-center gap-1 text-[9px] text-orange-400 mt-1 font-bold animate-pulse">
      ...
      {directionsSelectionTarget && (<div className="absolute top-16 left-1/2 -translate-x-1/2 z-[1600] pointer-events-none">
          <div className="px-5 py-2 rounded-full text-xs font-bold text-white shadow-2xl border border-indigo-400/40 animate-pulse" style={{ background: 'rgba(79,70,229,0.85)', backdropFilter: 'blur(12px)' }}>
      ...
      <div className="absolute top-4 right-4 z-[1000] flex flex-col gap-4 w-[220px]">
        ...
        <div className="glass-panel p-4 rounded-xl text-theme-fg shadow-xl flex flex-col gap-3">
        ...
        }} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all border ${showIncidentsHeat
        ...
        }} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all border ${showRatingsHeat
          ...
          <div className="mt-3 pt-3 border-t border-theme-border space-y-2">
            <button onClick={() => setIsDateModalOpen(true)} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-[10px] font-bold transition-all border ${dateRange.from
            ...
            {dateRange.from && dateLabel && (<div className="px-3 py-1.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20">
        ...
        <div className="flex flex-col gap-2">
          ...
          <button onClick={() => {
            ...
            setSelectionMode(null);
        }} className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-xs font-extrabold transition-all duration-200 hover:scale-[1.02] active:scale-95 shadow-lg backdrop-blur-md border ${directionsOpen
          ...
          <div className="grid grid-cols-2 gap-2 mt-1">
            ...
            return (<button key={type.id} onClick={() => {
                    if (isSelected) {
                        setSelectionMode(null);
                ...
                }} disabled={directionsOpen} title={directionsOpen ? 'Close Directions panel first' : undefined} className={`w-full ${isSelected ? 'bg-orange-200 shadow-orange-300/40 text-orange-800 border-orange-300/50' : 'bg-orange-100 hover:bg-orange-200 text-orange-700 border-orange-300/50'} px-2 py-2 rounded-lg text-[11px] font-extrabold transition-all duration-200 hover:scale-[1.02] active:scale-95 shadow-lg backdrop-blur-md flex flex-col items-center justify-center gap-1 border disabled:opacity-40 disabled:cursor-not-allowed disabled:scale-100 leading-tight text-center`}>
                  <span className="text-lg">{type.icon}</span>
                  {isSelected ? 'Cancel' : type.name}
                </button>);
        })}
          </div>
          <button onClick={() => setSelectionMode(selectionMode === 'rating' ? null : 'rating')} disabled={directionsOpen} title={directionsOpen ? 'Close Directions panel first' : undefined} className={`w-full ${selectionMode === 'rating' ? 'bg-blue-200 shadow-blue-300/40 text-blue-900 border-blue-300/50' : 'bg-blue-100 hover:bg-blue-200 text-blue-800 border-blue-300/50'} px-4 py-2.5 rounded-lg text-xs font-extrabold transition-all duration-200 hover:scale-[1.02] active:scale-95 shadow-lg backdrop-blur-md flex items-center justify-center gap-2 border disabled:opacity-40 disabled:cursor-not-allowed disabled:scale-100`}>
      ...
      {/* Directions Panel */}
      {directionsOpen && (<DirectionsPanel onMapSelectStart={(target) => {
      ...
      {showReportForm && selectedLocation && (<div className="absolute inset-0 z-[2000] bg-theme-bg-start backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">

          <div className="max-w-md w-full">
            <ReportForm location={{ lat: selectedLocation.lat, lng: selectedLocation.lng }} incidentTypes={incidentTypes} severityLevels={severityLevels} preselectedIncidentTypeId={selectedIncidentTypeId} onSuccess={() => {
      ...
      {showRatingForm && selectedLocation && (<div className="absolute inset-0 z-[2000] bg-theme-bg-start backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">

          <div className="max-w-md w-full">
            <StreetRatingForm location={{ lat: selectedLocation.lat, lng: selectedLocation.lng }} onSuccess={() => {
      ...
      <DateFilterModal isOpen={isDateModalOpen} onClose={() => setIsDateModalOpen(false)} onApply={(from, to, label) => {
            setDateRange({ from, to });
            setDateLabel(label ?? null);
        }} initialFrom={dateRange.from} initialTo={dateRange.to}/>
    </div>);
};
