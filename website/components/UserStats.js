...
const UserStats = ({ stats }) => {
    ...
    return (<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      {cards.map((card, i) => (<motion.div key={card.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="relative overflow-hidden rounded-2xl p-4 bg-theme-panel border border-theme-border shadow-lg group hover:border-slate-700 transition-colors">
