    ...
    return (<div className="p-6 glass-panel rounded-xl space-y-5 shadow-2xl max-w-md min-w-[320px]">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 text-sm">
        ...
        <div className="rounded-lg border border-slate-600 bg-theme-panel/60 p-3 space-y-3">
          <button type="button" onClick={() => setRateLighting(v => !v)} className={`w-full flex items-center justify-between text-sm font-semibold transition-colors ${rateLighting ? 'text-yellow-300' : 'text-theme-fg-muted'}`}>
        ...
        <div className="space-y-1.5">
          ...
          <textarea {...register('comment')} className="w-full bg-theme-panel border border-slate-600 rounded-lg p-2.5 text-theme-fg placeholder-slate-500 h-20 focus:ring-2 focus:ring-emerald-500 outline-none resize-none text-sm" placeholder="Any specific observations?"/>
        </div>
        ...
        <div className="flex space-x-3 pt-1">
          <button type="button" onClick={onCancel} className="flex-1 py-2.5 rounded-lg bg-slate-700 hover:bg-slate-600 font-semibold transition-colors text-theme-fg text-sm">
            Cancel
          </button>
          <button type="submit" disabled={isSubmitting || isUploading} className="flex-1 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 font-semibold transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-50 text-theme-fg text-sm">
