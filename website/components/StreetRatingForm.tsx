...
interface StreetRatingFormProps {
...
function getRatingInfo(val: number) {
...
function ScoreSlider({ label, fieldName, register, watch }: {
  label: string;
  fieldName: keyof RatingFormValues;
  register: any;
  watch: any;
}) {
          ...
          >
            ...
            <span className={`w-9 h-5 rounded-full transition-colors flex items-center px-0.5 ${
              ...
              <span className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${
                rateLighting ? 'translate-x-4' : 'translate-x-0'
              }`} />
            </span>
          </button>

          {rateLighting && (
            <ScoreSlider label="Lighting" fieldName="lighting_score" register={register} watch={watch} />
          )}
        </div>
