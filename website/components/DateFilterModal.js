...
import { startOfMonth, endOfMonth, addDays, startOfDay, endOfDay } from 'date-fns';
export const DateFilterModal = ({ isOpen, onClose, onApply, initialFrom, initialTo, }) => {
    ...
    const resolveDate = (week, dayIndex) => {
    ...
    return (<div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-theme-bg-start backdrop-blur-sm transition-all duration-300">
      <div className="relative w-full max-w-md bg-theme-panel/90 border border-slate-700/50 rounded-2xl shadow-2xl overflow-hidden glass-panel active-glow animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-theme-border">
          ...
          <button onClick={onClose} className="p-1 hover:bg-theme-panel rounded-lg transition-colors text-theme-fg-muted hover:text-theme-fg">
            <X className="w-6 h-6"/>
          </button>
        </div>
        ...
        <div className="flex p-1 m-4 bg-theme-panel/50 rounded-xl border border-slate-700/30">
          <button onClick={() => setMode('structured')} className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-all ${mode === 'structured'
          ...
          <button onClick={() => setMode('custom')} className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-all ${mode === 'custom'
            ? 'bg-indigo-600 text-theme-fg shadow-lg shadow-indigo-900/20'
            : 'text-theme-fg-muted hover:text-slate-200 hover:bg-theme-border-hover/50'}`}>
            Custom Range
          </button>
        ...
        <div className="p-5 max-h-[60vh] overflow-y-auto space-y-6">
          {mode === 'structured' ? (<div className="space-y-4">
              {/* Year Picker */}
              <div className="space-y-2">
                <label className="text-xs font-semibold text-theme-fg-muted uppercase tracking-wider">Select Year</label>
                <div className="flex items-center gap-2">
                  <select value={isCustomYear || !yearOptions.includes(selectedYear) ? 'custom' : selectedYear} onChange={(e) => {
                if (e.target.value === 'custom') {
                    ...
                    {yearOptions.map(yr => (<option className="bg-theme-panel text-slate-200" key={yr} value={yr}>{yr}</option>))}
                    <option className="bg-theme-panel text-slate-200" value="custom">Custom...</option>
                  </select>
                  {(isCustomYear || !yearOptions.includes(selectedYear)) && (<input type="number" value={selectedYear} onChange={(e) => {
                ...
                }} className="w-24 bg-theme-panel/50 border border-slate-700 text-theme-fg text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 outline-none p-2.5 transition-all text-center" placeholder="Year"/>)}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {months.map((month, index) => (<button key={month} onClick={() => {
                    ...
                    setSelectedWeek(null);
                    setSelectedDay(null);
                }} className={`py-2 text-xs rounded-lg border transition-all ${selectedMonth === index
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((week) => (<button key={week} onClick={() => {
                    setSelectedWeek(selectedWeek === week ? null : week);
                    setSelectedDay(null);
                }} className={`flex-1 py-2 text-xs rounded-lg border transition-all ${selectedWeek === week
                    ? 'bg-indigo-500/20 border-indigo-500 text-indigo-300'
                    : 'bg-theme-panel/30 border-slate-700 text-theme-fg-muted hover:bg-theme-panel hover:border-slate-600'}`}>
                      Week {week}
                    </button>))}
              ...
              {/* Day Picker */}
              {selectedWeek && (<div className="space-y-2 animate-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    const dateStr = resolved
                        ...
                        : null;
                    return (<button key={day} onClick={() => resolved ? setSelectedDay(selectedDay === index ? null : index) : undefined} disabled={!resolved} className={`flex items-center justify-between px-3 py-2 rounded-lg border transition-all ${!resolved
                          ...
                          {dateStr && (<span className={`text-[10px] font-mono ${selectedDay === index ? 'text-indigo-400' : 'text-theme-fg-muted'}`}>{dateStr}</span>)}
                        </button>);
                })}
                  </div>
                </div>)}
              ...
              <div className="space-y-2">
                ...
                <input type="date" value={customFrom} onChange={(e) => setCustomFrom(e.target.value)} className="w-full bg-theme-panel border border-slate-700 rounded-xl p-3 text-theme-fg focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all font-mono"/>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-semibold text-theme-fg-muted uppercase tracking-wider">End Date</label>
                <input type="date" value={customTo} onChange={(e) => setCustomTo(e.target.value)} className="w-full bg-theme-panel border border-slate-700 rounded-xl p-3 text-theme-fg focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all font-mono"/>
              </div>
            </div>)}
        </div>
        ...
        <div className="p-5 flex gap-3 border-t border-theme-border bg-theme-panel/50">
          <button onClick={handleReset} className="flex items-center justify-center gap-2 px-4 py-3 bg-theme-panel hover:bg-theme-border-hover text-theme-fg-muted rounded-xl font-medium transition-all">
            <RotateCcw className="w-4 h-4"/>
            Reset
          </button>
          <button onClick={handleApply} className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-theme-fg py-3 rounded-xl font-semibold shadow-lg shadow-indigo-900/40 transition-all active:scale-[0.98]">
            Apply Filter
          </button>
        </div>
      </div>
