...
export const DateFilterModal = ({ isOpen, onClose, onApply, initialFrom, initialTo, }) => {
    ...
    return (<div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm transition-all duration-300">
      <div className="relative w-full max-w-md bg-slate-900/90 border border-slate-700/50 rounded-2xl shadow-2xl overflow-hidden glass-panel active-glow animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
          ...
          {mode === 'structured' ? (<div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                if (e.target.value === 'custom') {
                    ...
                    {yearOptions.map(yr => (<option className="bg-slate-800 text-slate-200" key={yr} value={yr}>{yr}</option>))}
                ...
                }} className="w-24 bg-slate-800/50 border border-slate-700 text-white text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 outline-none p-2.5 transition-all text-center" placeholder="Year"/>)}
                </div>
              </div>
              ...
              {selectedWeek && (<div className="space-y-2 animate-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    return (<button key={day} onClick={() => resolved ? setSelectedDay(selectedDay === index ? null : index) : undefined} disabled={!resolved} className={`flex items-center justify-between px-3 py-2 rounded-lg border transition-all ${!resolved
                          ...
                          {dateStr && (<span className={`text-[10px] font-mono ${selectedDay === index ? 'text-indigo-400' : 'text-slate-500'}`}>{dateStr}</span>)}
                        </button>);
                })}
                  </div>
                </div>)}
              ...
              <div className="space-y-2">
                ...
                <input type="date" value={customFrom} onChange={(e) => setCustomFrom(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all font-mono"/>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">End Date</label>
                <input type="date" value={customTo} onChange={(e) => setCustomTo(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all font-mono"/>
              </div>
            </div>)}
        </div>
        ...
        <div className="p-5 flex gap-3 border-t border-slate-800 bg-slate-900/50">
          <button onClick={handleReset} className="flex items-center justify-center gap-2 px-4 py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl font-medium transition-all">
            <RotateCcw className="w-4 h-4"/>
            Reset
          </button>
          <button onClick={handleApply} className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-xl font-semibold shadow-lg shadow-indigo-900/40 transition-all active:scale-[0.98]">
            Apply Filter
          </button>
        </div>
      </div>
