...
const ReportForm = ({ location, incidentTypes, severityLevels, preselectedIncidentTypeId, onSuccess, onCancel }) => {
    const { token } = useAuth();
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL || '/api/v1'}/reports`, {
                ...data,
    ...
    return (<div className="p-6 glass-panel rounded-xl space-y-4 shadow-2xl min-w-[320px]">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="space-y-1">
          ...
          {selectedIncidentType ? (<div className="flex items-center gap-2 p-2.5 bg-theme-panel/50 border border-slate-700 rounded-lg text-theme-fg font-medium">
              <span className="text-xl">{selectedIncidentType.icon}</span>
              <span>{selectedIncidentType.name}</span>
            </div>) : (<select {...register('incident_type_id')} className="w-full bg-theme-panel border border-slate-700 rounded-lg p-2 text-theme-fg focus:ring-2 focus:ring-blue-500 outline-none">
              <option value="">Select Type</option>
              {incidentTypes.map(t => (<option key={t.id} value={t.id}>{t.icon} {t.name}</option>))}
            </select>)}
        ...
        <div className="space-y-1">
          <div className="flex justify-between items-baseline">
            ...
            {selectedSeverity && (<span className="text-sm font-bold" style={{ color: selectedSeverity.color_code }}>
          ...
          <div className="flex gap-2">
            ...
            return (<button key={level.id} type="button" onClick={() => setValue('severity_level_id', level.id)} className="flex-1 py-2 text-xs font-bold rounded-lg border transition-all" style={{
        ...
        <div className="space-y-1">
          ...
          <textarea {...register('description')} className="w-full bg-theme-panel border border-slate-700 rounded-lg p-2 text-theme-fg h-24 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Provide details about the incident..."/>
        ...
        <div className="flex space-x-3 pt-2">
          <button type="button" onClick={onCancel} className="flex-1 py-2 rounded-lg bg-theme-panel hover:bg-theme-border-hover font-semibold transition-colors">
            Cancel
          </button>
          <button type="submit" disabled={isSubmitting || isUploading} className="flex-2 px-6 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 font-semibold transition-all shadow-lg shadow-blue-500/20 disabled:opacity-50">
