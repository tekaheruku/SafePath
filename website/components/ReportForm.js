...
const ReportForm = ({ location, onSuccess, onCancel }) => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api/v1'}/reports`, {
                ...data,
    ...
    return (<div className="p-6 glass-panel rounded-xl space-y-4 shadow-2xl min-w-[320px]">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="space-y-1">
          ...
          <select {...register('type')} className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2 text-white focus:ring-2 focus:ring-blue-500 outline-none">
        ...
        <div className="space-y-1">
          ...
          <input type="range" min="1" max="5" step="1" {...register('severity_level')} className="w-full accent-blue-500"/>
        ...
        <div className="space-y-1">
          ...
          <textarea {...register('description')} className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2 text-white h-24 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Provide details about the incident..."/>
          {errors.description && <p className="text-xs text-red-400">{errors.description.message}</p>}
        </div>

        <div className="flex space-x-3 pt-2">
          <button type="button" onClick={onCancel} className="flex-1 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 font-semibold transition-colors">
            Cancel
          </button>
          <button type="submit" disabled={isSubmitting} className="flex-2 px-6 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 font-semibold transition-all shadow-lg shadow-blue-500/20 disabled:opacity-50">
