...
const UserReportList = ({ reports, onDelete, onView }) => {
    return (<div className="grid gap-4 md:grid-cols-2">
      <AnimatePresence>
        {reports.map((r, i) => (<motion.div key={r.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: i * 0.05 }} layout onClick={() => onView(r.id, r.location.coordinates[1], r.location.coordinates[0])} className="group glass-panel bg-theme-panel/40 p-5 rounded-2xl cursor-pointer hover:bg-theme-panel/60 hover:border-white/20 transition-all border border-theme-border active:scale-[0.98]">
            <div className="flex justify-between items-start mb-3">
              ...
              <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${r.severity_level === 'high' ? 'bg-red-500/10 border-red-500/30 text-red-500' :
