...
const UserReportList = ({ reports, onDelete, onView }) => {
    return (<div className="grid gap-4 md:grid-cols-2">
      <AnimatePresence>
        {reports.map((r, i) => (<motion.div key={r.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: i * 0.05 }} layout onClick={() => onView(r.id, r.location.coordinates[1], r.location.coordinates[0])} className="group glass-panel bg-slate-900/40 p-5 rounded-2xl cursor-pointer hover:bg-slate-800/60 hover:border-white/20 transition-all border border-white/5 active:scale-[0.98]">
