...
import { Navigation, Footprints, Bike, Car, X, RotateCcw, ArrowUpDown, MapPin, Clock, Ruler, Shield, ChevronRight, AlertCircle, MousePointerClick, CheckCircle2, Star, Zap, Info } from 'lucide-react';
...
function safetyColor(score, hasRatings) {
    if (!hasRatings)
        return { stroke: '#818cf8', label: 'Unrated', bg: 'bg-indigo-500/15', ring: 'ring-indigo-500/40' };
    if (score >= 4.0)
        return { stroke: '#22c55e', label: 'Safest', bg: 'bg-emerald-500/15', ring: 'ring-emerald-500/40' };
    if (score >= 2.5)
        return { stroke: '#f59e0b', label: 'Moderate', bg: 'bg-amber-500/15', ring: 'ring-amber-500/40' };
    return { stroke: '#ef4444', label: 'Caution', bg: 'bg-red-500/15', ring: 'ring-red-500/40' };
}
function safetyBarWidth(score) {
...
const InputField = ({ id, label, point, isSelectingMap, localData, onSelectPoint, onClear, onStartMapSelect }) => {
    ...
    return (<div ref={containerRef} className="relative">
      <div className={`flex items-center gap-2 rounded-xl border px-3 py-3 transition-all ${isSelectingMap
            ? 'border-indigo-400/60 bg-indigo-500/10 ring-1 ring-indigo-500/30'
            : 'border-white/10 bg-white/5 hover:border-white/20'}`}>
        {Icon}
        <input type="text" value={isSelectingMap ? '📍 Click map to place pin…' : query} onChange={(e) => {
            if (isSelectingMap)
                return;
            if (point)
                onClear(); // typing clears the locked point
            setQuery(e.target.value);
        }} onFocus={() => suggestions.length > 0 && !point && setShowSugg(true)} readOnly={isSelectingMap} placeholder={label} className={`flex-1 bg-transparent text-sm outline-none placeholder:text-white/35 min-w-0 ${isSelectingMap ? 'text-indigo-300 cursor-default' : 'text-white'}`} style={{ color: isSelectingMap ? undefined : 'white', caretColor: 'white' }}/>
        {/* Map-click button */}
        {!point && !isSelectingMap && (<button onClick={onStartMapSelect} title="Click on map to select" className="flex-shrink-0 p-1 hover:bg-white/10 rounded-lg transition-colors text-white/50 hover:text-indigo-300">
            <MousePointerClick className="w-3.5 h-3.5"/>
          </button>)}
        {/* Clear / cancel */}
        {(point || isSelectingMap) && (<button onClick={onClear} className="flex-shrink-0 p-1 hover:bg-white/10 rounded-lg transition-colors text-white/40 hover:text-white">
            <X className="w-3.5 h-3.5"/>
          </button>)}
      </div>

      {/* Autocomplete dropdown */}
      {showSugg && !point && (<div className="absolute top-full left-0 right-0 mt-1.5 z-10 rounded-xl border border-white/10 bg-slate-900/95 backdrop-blur-xl shadow-2xl overflow-hidden max-h-52 overflow-y-auto">
...
const RouteCard = ({ route, rank, isSelected, isRecommended, recommendedLabel, onClick }) => {
    const [expanded, setExpanded] = useState(false);
    const color = safetyColor(route.safetyScore, route.hasRatings);
    return (<div onClick={onClick} className={`cursor-pointer rounded-xl border transition-all duration-200 overflow-hidden ${isSelected
            ? `${color.bg} ring-1 ${color.ring} border-white/20`
            : 'border-white/8 bg-white/3 hover:bg-white/6 hover:border-white/15'}`}>
      {/* Recommended badge */}
      {isRecommended && (<div className="flex items-center gap-1.5 px-3 py-2 border-b border-white/8" style={{ background: `${color.stroke}18` }}>
      ...
      <div className="p-3.5 flex items-center gap-3">
        {/* Rank badge */}
        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-sm font-black border ${isSelected ? 'bg-white/20 border-white/30 text-white' : 'bg-white/5 border-white/10 text-white/60'}`}>
          {rank}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            {/* Safety badge */}
            <span className="px-2 py-0.5 rounded-full text-xs font-black tracking-wide" style={{ backgroundColor: color.stroke + '25', color: color.stroke }}>
              {color.label}
            </span>

            {/* Unrated info icon or score stars */}
            {route.hasRatings ? (<div className="flex items-center gap-0.5">
                {[1, 2, 3, 4, 5].map(n => (<Star key={n} className="w-3 h-3" style={{ color: n <= Math.round(route.safetyScore) ? color.stroke : '#ffffff20', fill: n <= Math.round(route.safetyScore) ? color.stroke : 'transparent' }}/>))}
                <span className="text-xs font-bold ml-1" style={{ color: color.stroke }}>
                  {route.safetyScore.toFixed(1)}
                </span>
              </div>) : (<span className="text-xs text-indigo-300/70 font-medium flex items-center gap-1">
          ...
          {/* Safety bar */}
          {route.hasRatings && (<div className="mt-2 h-1 rounded-full bg-white/8 overflow-hidden">
              <div className="h-full rounded-full transition-all duration-500" style={{ width: safetyBarWidth(route.safetyScore), backgroundColor: color.stroke }}/>
            </div>)}
        </div>

        {/* Expand arrow */}
        <button onClick={(e) => { e.stopPropagation(); setExpanded(!expanded); }} className="flex-shrink-0 p-1 hover:bg-white/10 rounded-lg transition-all text-white/40 hover:text-white">
          <ChevronRight className={`w-4 h-4 transition-transform duration-200 ${expanded ? 'rotate-90' : ''}`}/>
        </button>
      </div>

      {/* Expanded breakdown */}
      {expanded && (<div className="px-3 pb-3 pt-0 border-t border-white/8">
          {route.hasRatings ? (<div className="mt-2.5 grid grid-cols-2 gap-2">
              {[
                    ...
                    return (<div key={label} className="flex items-center justify-between bg-white/4 rounded-lg px-2.5 py-2">
            ...
            </div>) : (<div className="mt-2.5 p-3.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20">
          ...
          {route.hasRatings && (<p className="text-[11px] text-white/35 mt-2 text-center">
              Based on {route.breakdown.ratedSegmentCount} community rating{route.breakdown.ratedSegmentCount !== 1 ? 's' : ''} along this route
            </p>)}
        </div>)}
    </div>);
};
const DirectionsPanel = ({ onMapSelectStart, onCancelMapSelect, onRoutesFetched, onClose, onRouteSelected, }) => {
    const { profile, setProfile, routeMode, setRouteMode, startPoint, setStartPoint, endPoint, setEndPoint, routes, setRoutes, selectedRouteIndex, setSelectedRouteIndex, selectionTarget, setSelectionTarget, isLoading, setLoading, error, setError, safestRecommendedIndex, balancedRecommendedIndex, clear, } = useDirectionsStore();
    ...
    /* ── Route fetching ───────────────────────────────────────────────────── */
    const fetchRoutes = useCallback(async (start, end, activeProfile, activeMode) => {
        setLoading(true);
        setError(null);
        setRoutes([], 0, 0);
        try {
            ...
            const routesToScore = osrmData.routes.map((r, i) => ({
                index: i,
    ...
    /* ── Handlers ─────────────────────────────────────────────────────────── */
    const handleStartMapSelect = (target) => {
        setSelectionTarget(target);
        onMapSelectStart(target);
    };
    const handleCancelMapSelect = (target) => {
    ...
    const handleRouteCardClick = (rank) => {
    ...
    return (<div className="absolute top-4 left-4 z-[1500] w-[360px] flex flex-col gap-2 max-h-[calc(100vh-6rem)] pointer-events-none">
      {/* Panel */}
      <div className="pointer-events-auto rounded-2xl border border-white/10 shadow-2xl overflow-hidden flex flex-col" style={{
        ...
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/8 flex-shrink-0">
          ...
          <button onClick={handleClose} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors text-white/40 hover:text-white">
            <X className="w-4 h-4"/>
          </button>
        </div>
        ...
        <div className="flex gap-1.5 px-3 pt-3 flex-shrink-0">
          {PROFILES.map(p => (<button key={p.id} onClick={() => setProfile(p.id)} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all ${profile === p.id
        ...
        <div className="px-3 pt-3 flex-shrink-0">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            return (<button key={mode.id} onClick={() => handleRouteModeChange(mode.id)} className={`relative flex flex-col items-start gap-1 px-3 py-2.5 rounded-xl text-left transition-all border ${isActive
                    ? 'text-white'
                    : 'border-white/8 bg-white/3 text-white/50 hover:bg-white/6 hover:text-white/70'}`} style={isActive ? { borderColor, background: bgColor } : {}}>
                  {isActive && (<div className="absolute top-2 right-2 w-2 h-2 rounded-full" style={{ backgroundColor: borderColor }}/>)}
        ...
        <div className="px-3 pt-3 pb-2 flex-shrink-0">
          <div className="flex gap-2">
            ...
            <div className="flex-1 flex flex-col gap-2 min-w-0">
              <InputField id="start" label="Choose starting point" point={startPoint} isSelectingMap={selectionTarget === 'start'} localData={localData} onSelectPoint={setStartPoint} onClear={() => {
            setStartPoint(null);
            if (selectionTarget === 'start') {
              ...
              <InputField id="end" label="Choose destination" point={endPoint} isSelectingMap={selectionTarget === 'end'} localData={localData} onSelectPoint={setEndPoint} onClear={() => {
            setEndPoint(null);
            ...
            <div className="flex flex-col justify-center flex-shrink-0">
              <button onClick={handleSwap} disabled={!startPoint && !endPoint} className="p-2 hover:bg-white/10 rounded-xl transition-all text-white/30 hover:text-white disabled:opacity-20 disabled:cursor-not-allowed" title="Swap start and end">
          ...
          {/* Clear all */}
          {(startPoint || endPoint) && (<button onClick={clear} className="mt-2 w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-bold text-white/35 hover:text-red-400 hover:bg-red-500/10 transition-all border border-transparent hover:border-red-500/20">
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-3" style={{ minHeight: 0 }}>
          ...
          {/* Loading */}
          {isLoading && (<div className="flex flex-col items-center justify-center py-10 gap-3">
          ...
          {/* Error */}
          {!isLoading && error && (<div className="flex flex-col items-center gap-3 py-6 text-center">
              <div className="p-2.5 rounded-full bg-red-500/10">
                <AlertCircle className="w-6 h-6 text-red-400"/>
              </div>
              <p className="text-sm text-red-300/80 leading-relaxed px-2">{error}</p>
              {startPoint && endPoint && (<button onClick={() => fetchRoutes(startPoint, endPoint, profile, routeMode)} className="text-sm font-bold text-indigo-400 hover:text-indigo-300 transition-colors">
                  Try again →
                </button>)}
            </div>)}

          {/* Prompt */}
          {!isLoading && !error && routes.length === 0 && (<div className="flex flex-col items-center gap-4 py-8 text-center px-2">
            ...
            ].map(({ c, l }) => (<div key={l} className="flex flex-col items-center gap-1.5 px-2 py-2.5 rounded-xl bg-white/3 border border-white/6">
          ...
          {/* Route cards */}
          {!isLoading && !error && routes.length > 0 && (<div className="flex flex-col gap-2.5">
              ...
              {routes.map((route, i) => (<RouteCard key={route.index} route={route} rank={i + 1} isSelected={selectedRouteIndex === i} isRecommended={i === currentRecommendedIndex} recommendedLabel={routeMode === 'safest'
                    ? (!route.hasRatings ? '✦ Recommended — Unrated (neutral)' : '✦ Recommended — Highest Safety')
                    : '✦ Recommended — Shortest Route'} onClick={() => handleRouteCardClick(i)}/>))}
