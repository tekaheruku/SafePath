    ...
    return (<div className="max-w-5xl mx-auto py-8">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 space-y-2">
          ...
          {user && (<button onClick={() => setActiveTab('security')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium text-sm ${activeTab === 'security'
          ...
          {activeTab === 'appearance' && (<div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {['white', 'dark-blue', 'dark-slate', 'dark'].map((t) => (<button key={t} onClick={() => setTheme(t)} className={`flex flex-col items-center gap-4 p-6 rounded-2xl border transition-all ${theme === t
                    ...
                    <div className={`w-16 h-16 rounded-full border-4 shadow-xl ${t === 'dark' ? 'bg-[#000000] border-theme-border' :
                ...
                <div className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h3 className="text-lg font-bold text-theme-fg flex items-center gap-2">
                      ...
                      {is2faEnabled ? (<span className="bg-emerald-500/20 text-emerald-400 text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider font-bold border border-emerald-500/30">Active</span>) : (<span className="bg-slate-500/20 text-theme-fg-muted text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider font-bold border border-slate-500/30">Disabled</span>)}
                    </h3>
              ...
              <section className="space-y-6">
                ...
                <form onSubmit={initiatePasswordChange} className="max-w-md space-y-4">
                  ...
                  {!is2faEnabled && !otpMode && (<div className="space-y-2">
                      ...
                      <input type="password" required value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} className="w-full bg-theme-bg-start border border-theme-border rounded-xl p-3 text-sm text-theme-fg focus:ring-2 focus:ring-indigo-500/50" placeholder="••••••••"/>
                    </div>)}
                  ...
                  {otpMode && (<div className="space-y-2">
                      ...
                      <input type="text" required maxLength={6} value={otpToken} onChange={(e) => setOtpToken(e.target.value)} className="w-full bg-indigo-500/10 border border-indigo-500/50 rounded-xl p-3 text-sm text-center tracking-[0.5em] font-mono font-bold text-theme-fg focus:ring-2 focus:ring-indigo-500" placeholder="XXXXXX"/>
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="password" required value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full bg-theme-bg-start border border-theme-border rounded-xl p-3 text-sm text-theme-fg focus:ring-2 focus:ring-indigo-500/50" placeholder="••••••••"/>
                  </div>

                  <div className="space-y-2">
                    <label className="text-xs text-theme-fg-muted uppercase tracking-wider font-bold">Confirm New Password</label>
                    <input type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full bg-theme-bg-start border border-theme-border rounded-xl p-3 text-sm text-theme-fg focus:ring-2 focus:ring-indigo-500/50" placeholder="••••••••"/>
                  </div>
                  ...
                  <div className="pt-2">
                    <button type="submit" disabled={isSubmitting} className="w-full bg-indigo-600 hover:bg-indigo-500 text-theme-fg font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-indigo-600/20 active:scale-[0.98] disabled:opacity-50">
                      {isSubmitting ? 'Processing...' : (otpMode ? 'Verify & Change Password' : 'Change Password')}
                    </button>
                    {otpMode && (<button type="button" onClick={() => { setOtpMode(false); setOtpToken(''); setMessage(null); }} className="w-full mt-3 text-theme-fg-muted hover:text-theme-fg text-sm py-2">
                        Cancel
                      </button>)}
                  </div>
