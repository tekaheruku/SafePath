...
export default function AccountListPage() {
    ...
    const processedUsers = React.useMemo(() => {
        let items = [...users].filter(u => u.name.toLowerCase().includes(search.toLowerCase()) ||
    ...
    if (authLoading || !user || (user.role !== 'superadmin' && user.role !== 'lgu_admin'))
        return null;
    return (<div className="min-h-screen bg-theme-bg-start text-theme-fg p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <div className="relative w-full md:w-72">
            <input type="text" placeholder="Search users..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-theme-panel border border-theme-border rounded-xl px-4 py-2.5 pl-10 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"/>
        ...
        {error && (<div className="bg-red-500/10 border border-red-500/50 text-red-400 p-4 rounded-xl mb-6 flex items-center space-x-3">
                  ...
                  <th className="px-6 py-4 font-semibold">
                    <div className="flex items-center space-x-4">
                       ...
                       <div className="flex items-center space-x-2">
                         <button onClick={(e) => { e.stopPropagation(); requestSort('first_name'); }} className={`flex items-center space-x-1 px-1.5 py-0.5 rounded border transition-all ${sortConfig?.key === 'first_name'
            ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-400'
            : 'border-slate-700 text-theme-fg-muted hover:text-theme-fg-muted'}`} title="Sort by First Name">
                           <span className="text-[10px]">FN</span>
                           <span className="text-[10px]">{sortConfig?.key === 'first_name' ? (sortConfig.direction === 'asc' ? '▴' : '▾') : '▴'}</span>
                         </button>
                         <button onClick={(e) => { e.stopPropagation(); requestSort('last_name'); }} className={`flex items-center space-x-1 px-1.5 py-0.5 rounded border transition-all ${sortConfig?.key === 'last_name'
            ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-400'
            : 'border-slate-700 text-theme-fg-muted hover:text-theme-fg-muted'}`} title="Sort by Last Name">
                  ...
                  <th className="px-6 py-4 font-semibold cursor-pointer group hover:bg-theme-panel/50 transition-colors" onClick={() => requestSort('role')}>
                  ...
                  <th className="px-6 py-4 font-semibold cursor-pointer group hover:bg-theme-panel/50 transition-colors" onClick={() => requestSort('created_at')}>
                    <div className="flex items-center space-x-1">
                      <span>Created</span>
              ...
              <tbody className="divide-y divide-slate-800">
                {loading ? (Array(5).fill(0).map((_, i) => (<tr key={i} className="animate-pulse">
                      <td colSpan={6} className="px-6 py-6 h-16 bg-theme-panel/50"></td>
                    </tr>))) : processedUsers.length > 0 ? (processedUsers.map((u) => (<tr key={u.id} className="hover:bg-theme-panel/30 transition-colors cursor-pointer group" onClick={() => setSelectedUser(u)}>
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-tight ${u.role === 'superadmin' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' :
                u.role === 'lgu_admin' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' :
                    'bg-slate-500/10 text-theme-fg-muted border border-slate-500/20'}`}>
                      ...
                      <td className="px-6 py-4">
                        {u.banned_until && new Date(u.banned_until) > new Date() ? (<span className="flex items-center text-red-400 text-xs">
                            <div className="w-1.5 h-1.5 bg-red-500 rounded-full mr-2"></div>
                            Banned
                          </span>) : (<span className="flex items-center text-emerald-400 text-xs">
