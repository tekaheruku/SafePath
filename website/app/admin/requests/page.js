...
export default function AdminRequestsPage() {
    ...
    const handleApprove = async (id, name) => {
        if (!window.confirm(`Are you sure you want to approve ${name}'s request? They will be granted immediate access.`))
            return;
        try {
    ...
    return (<div className="min-h-screen bg-theme-bg-start text-theme-fg p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {error && (<div className="bg-red-500/10 border border-red-500/50 text-red-400 p-4 rounded-xl mb-6">
            <span>{error}</span>
          </div>)}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-800">
                {loading ? (Array(3).fill(0).map((_, i) => (<tr key={i} className="animate-pulse">
                      <td colSpan={6} className="px-6 py-6 h-16 bg-theme-panel/50"></td>
                    </tr>))) : requests.length > 0 ? (requests.map((r) => (<tr key={r.id} className="hover:bg-theme-panel/30 transition-colors">
                      <td className="px-6 py-4">
                        <div className="font-semibold text-white">{r.name}</div>
                        <div className="text-xs text-theme-fg-muted">{r.email}</div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-tight ${r.requested_role === 'superadmin' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' :
                'bg-blue-500/10 text-blue-400 border border-blue-500/20'}`}>
                      ...
                      <td className="px-6 py-4 text-sm text-theme-fg-muted max-w-xs break-words">
                        ...
                        {r.document_url && (<div className="mt-2">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-md text-xs font-bold uppercase ${r.status === 'approved' ? 'text-emerald-400 bg-emerald-500/10' :
                r.status === 'rejected' ? 'text-red-400 bg-red-500/10' :
                    'text-yellow-400 bg-yellow-500/10'}`}>
                      ...
                      <td className="px-6 py-4 text-right">
                        {r.status === 'pending' && (<div className="flex items-center justify-end space-x-2">
                            <button onClick={() => handleApprove(r.id, r.name)} className="px-3 py-1.5 text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg transition-transform hover:scale-105">
                              Approve
                            </button>
                            <button onClick={() => handleReject(r.id)} className="px-3 py-1.5 text-xs font-semibold bg-red-900/50 hover:bg-red-800/60 text-red-100 rounded-lg transition-transform hover:scale-105">
                              Reject
                            </button>
                          </div>)}
                      </td>
