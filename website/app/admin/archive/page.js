...
import { Calendar, FilterX, RotateCcw, Archive } from 'lucide-react';
import { ADMIN_ROLES, REPORT_REVIEW_ACTIONS, APP_ROUTES } from '@safepath/shared';
export default function AdminArchivePage() {
    ...
    const handleRestore = async (e, reportId) => {
        e.stopPropagation();
        if (!token)
            return;
        ...
        try {
            await axios.post(`${apiUrl}/reports/${reportId}/restore`, {}, { headers: { Authorization: `Bearer ${token}` } });
    ...
    return (<div className="container mx-auto px-4 py-12 max-w-4xl">
      ...
      {feedbackMessage && (<div className={`mb-6 p-4 rounded-xl text-sm font-semibold border flex items-center justify-between ${feedbackMessage.type === 'success'
                ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
          <span>{feedbackMessage.text}</span>
          <button onClick={() => setFeedbackMessage(null)} className="text-xs uppercase tracking-wider font-bold opacity-70 hover:opacity-100 ml-4">
            Dismiss
          </button>
        </div>)}
      ...
      <div className="bg-theme-panel rounded-2xl border border-theme-border p-4 mb-8 flex flex-col md:flex-row gap-4 items-end shadow-md">
        <div className="flex flex-col gap-1.5 w-full md:w-1/2">
          ...
          <select value={selectedType} onChange={(e) => setSelectedType(e.target.value)} className="w-full bg-slate-900 border border-theme-border rounded-lg px-3 py-2.5 text-sm text-white outline-none focus:border-indigo-500 transition-colors cursor-pointer">
            <option value="" className="bg-slate-900 text-white">All Types</option>
            {incidentTypes.map(type => (<option key={type.id} value={type.id} className="bg-slate-900 text-white">{type.name}</option>))}
          </select>
        </div>
        ...
        <div className="flex flex-col gap-1.5 w-full md:w-1/2">
          ...
          <div className="flex gap-2">
            <button onClick={() => setIsDateModalOpen(true)} className={`flex-1 flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-semibold transition-all border ${dateRange.from
            ...
            {dateRange.from && (<button onClick={() => {
                ...
                setDateLabel(null);
            }} className="px-3 py-2.5 bg-theme-bg-start border border-theme-border hover:border-red-500/50 hover:bg-red-500/10 text-theme-fg-muted hover:text-red-400 rounded-lg transition-colors" title="Clear Date Filter">
      ...
      {loading ? (<div className="bg-theme-panel rounded-2xl border border-theme-border p-12 text-center text-theme-fg-muted">
        ...
        </div>) : reports.length === 0 ? (<div className="bg-theme-panel rounded-2xl border border-theme-border p-12 text-center text-theme-fg-muted">
          ...
          {reports.map((r) => (<div key={r.id} onClick={() => handleReportClick(r)} className="bg-theme-panel border border-theme-border p-5 rounded-2xl cursor-pointer hover:border-slate-600 transition-all shadow-lg flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start mb-2 gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${(r.severity_level_name ?? r.severity_level) === 'Critical' ? 'bg-red-500/20 text-red-400' :
                ...
                {r.photo_url && (<div className="mb-3">
                    <img src={r.photo_url} alt="Archived Report" className="w-full h-32 object-cover rounded-lg border border-theme-border opacity-75"/>
                  </div>)}
              ...
              <div className="pt-3 border-t border-theme-border flex items-center justify-between gap-3 text-xs text-theme-fg-muted">
                ...
                <button onClick={(e) => handleRestore(e, r.id)} disabled={actionLoading === r.id} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-md shadow-indigo-600/30 active:scale-95 transition-all disabled:opacity-50" title="Restore to Pending Review">
