...
export default function RequestAdminPage() {
    ...
    if (success) {
        return (<div className="min-h-screen bg-theme-bg-start flex items-center justify-center p-4">
    ...
    return (<div className="min-h-screen bg-theme-bg-start flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-theme-panel border border-theme-border rounded-3xl p-8 shadow-2xl">
        ...
        {error && (<div className="bg-red-500/10 border border-red-500/50 text-red-400 p-4 rounded-xl mb-6 text-sm">
            {error}
          </div>)}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input type="text" required placeholder="e.g. Juan De La Cruz" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full bg-theme-bg-start/50 border border-theme-border rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition-all placeholder:text-slate-600 focus:bg-theme-panel"/>
          </div>

          <div>
            <label className="block text-sm font-semibold text-slate-300 mb-2">Work Email</label>
            <input type="email" required placeholder="you@gov.ph" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full bg-theme-bg-start/50 border border-theme-border rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition-all placeholder:text-slate-600 focus:bg-theme-panel"/>
          </div>

          <div>
            <label className="block text-sm font-semibold text-slate-300 mb-2">Requested Role</label>
            <div className="grid grid-cols-2 gap-4">
              <button type="button" onClick={() => setFormData({ ...formData, requestedRole: 'lgu_admin' })} className={`flex flex-col items-center justify-center p-4 rounded-xl border text-sm font-semibold transition-all ${formData.requestedRole === 'lgu_admin'
            ...
            : 'bg-theme-bg-start/50 border-theme-border text-slate-400 hover:bg-theme-panel'}`}>
              ...
              <button type="button" onClick={() => setFormData({ ...formData, requestedRole: 'superadmin' })} className={`flex flex-col items-center justify-center p-4 rounded-xl border text-sm font-semibold transition-all ${formData.requestedRole === 'superadmin'
          ...
          <div>
            ...
            <textarea required rows={4} placeholder="Please briefly explain why you need these elevated privileges..." value={formData.reason} onChange={(e) => setFormData({ ...formData, reason: e.target.value })} className="w-full bg-theme-bg-start/50 border border-theme-border rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition-all placeholder:text-slate-600 focus:bg-theme-panel resize-none"/>
          </div>
          ...
          <div>
            ...
            <input type="file" required accept="image/*,application/pdf" onChange={(e) => setDocument(e.target.files?.[0] || null)} className="w-full text-sm text-slate-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-indigo-500/10 file:text-indigo-400 hover:file:bg-indigo-500/20 file:transition-colors bg-theme-bg-start/50 border border-theme-border rounded-xl cursor-pointer focus:outline-none"/>
          </div>

          <button type="submit" disabled={loading} className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold tracking-wide shadow-lg shadow-indigo-600/30 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed">
