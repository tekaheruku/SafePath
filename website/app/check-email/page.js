...
function CheckEmailContent() {
    ...
    return (<div className="min-h-screen flex items-center justify-center bg-slate-950 p-4">
      <div className="max-w-md w-full text-center">
        ...
        {email && (<p className="text-blue-400 font-semibold text-base mb-6 break-all">{email}</p>)}
        ...
        <div className="space-y-3">
          ...
          <button id="resend-verification-btn" onClick={handleResend} disabled={resending || cooldown > 0} className="w-full bg-slate-800 hover:bg-slate-700 text-white font-semibold py-3.5 rounded-xl border border-slate-700 hover:border-slate-600 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
            {resending ? (<span className="flex items-center justify-center gap-2">
                ...
                Sending...
              </span>) : cooldown > 0 ? (`Resend in ${cooldown}s`) : ('Resend verification email')}
          </button>

          <a href="/login" className="block text-center text-slate-500 hover:text-slate-300 text-sm font-medium transition-colors py-2">
            Back to sign in
          </a>
        </div>
