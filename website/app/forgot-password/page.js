...
export default function ForgotPasswordPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api/v1'}/auth/request-password-reset`, { email });
        }
        catch {
    ...
    return (<div className="min-h-screen flex items-center justify-center bg-slate-950 p-4">
      <div className="max-w-md w-full">
        ...
        {!submitted ? (<div className="bg-slate-900 rounded-2xl p-8 border border-slate-800 shadow-2xl">
            {/* Header */}
            <div className="mb-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-1.5">
                ...
                <input id="forgot-email" type="email" required className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:ring-2 focus:ring-amber-500 outline-none transition-all" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)}/>
              </div>

              <button id="reset-request-btn" type="submit" disabled={loading} className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white font-bold py-3.5 rounded-xl transition-all duration-200 hover:shadow-lg hover:shadow-amber-500/25 active:scale-[0.98] disabled:opacity-50 mt-2">
        ...
        <div className="text-center">
            ...
            <a href="/login" className="inline-block bg-slate-800 hover:bg-slate-700 text-white font-semibold px-8 py-3 rounded-xl border border-slate-700 hover:border-slate-600 transition-all duration-200">
              Back to sign in
            </a>
          </div>)}
