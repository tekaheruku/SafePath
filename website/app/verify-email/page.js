...
function VerifyEmailContent() {
    ...
    const verify = useCallback(async () => {
        ...
        try {
            const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || '/api/v1'}/auth/verify-email?token=${token}`);
      ...
      <div className="max-w-md w-full text-center">
        ...
        {status === 'success' && (<div>
            ...
            <div className="space-y-3">
              <button id="goto-home-btn" onClick={() => router.push('/')} className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-theme-fg font-bold py-3.5 rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25">
                Go to SafePath
              </button>
        ...
        {status === 'error' && (<div>
            ...
            <div className="space-y-3">
              <a href="/register" className="block w-full bg-theme-panel hover:bg-theme-border-hover text-theme-fg font-semibold py-3.5 rounded-xl border border-slate-700 hover:border-slate-600 transition-all duration-200 text-center">
                Create a new account
              </a>
              <a href="/check-email" className="block text-center text-theme-fg-muted hover:text-theme-fg-muted text-sm font-medium transition-colors py-2">
