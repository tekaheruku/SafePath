...
export async function GET(request) {
    ...
    if (!startLat || !startLng || !endLat || !endLng) {
        return NextResponse.json({ success: false, error: 'Missing required params: startLat, startLng, endLat, endLng' }, { status: 400 });
    }
    ...
    const osrmUrl = `${osrmBase}/route/v1/${safeProfile}/${startLng},${startLat};${endLng},${endLat}` +
    ...
    try {
        ...
        if (!res.ok) {
            ...
            return NextResponse.json({ success: false, error: `OSRM returned ${res.status}` }, { status: 502 });
        }
        ...
        return NextResponse.json({ success: false, error: 'Routing service unavailable. Try again shortly.' }, { status: 502 });
    }
}
