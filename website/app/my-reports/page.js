...
export default function MyReportsPage() {
    ...
    return (<div className="container mx-auto px-4 py-12 max-w-4xl">
      ...
      {loading ? (<div className="bg-theme-panel rounded-2xl border border-theme-border p-8 text-center text-theme-fg-muted">
          <p className="animate-pulse">Loading your report history...</p>
        </div>) : reports.length === 0 ? (<div className="bg-theme-panel rounded-2xl border border-theme-border p-8 text-center text-theme-fg-muted">
          <p>You haven't submitted any incident reports yet.</p>
        </div>) : (<div className="grid gap-4 md:grid-cols-2">
          {reports.map((r) => (<div key={r.id} onClick={() => handleReportClick(r)} className="bg-theme-panel border border-theme-border p-5 rounded-2xl cursor-pointer hover:bg-theme-panel hover:border-slate-600 transition-all shadow-lg active:scale-[0.98]">
