...
export default function ReportPage() {
    ...
    return (<div className="container mx-auto px-4 py-10 max-w-4xl">
      ...
      {loading && (<div className="text-slate-400 text-center py-16 animate-pulse text-sm">Loading reports…</div>)}

      {!loading && error && (<div className="bg-slate-900 rounded-2xl border border-slate-800 p-12 text-center space-y-4">
          <p className="text-slate-300 text-base">No reports to display yet.</p>
          <p className="text-slate-500 text-sm">{error}</p>
          <button onClick={load} className="mt-2 px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-sm font-semibold text-white transition-colors">
            Try Again
          </button>
        </div>)}

      {!loading && !error && reports.length === 0 && (<div className="bg-slate-900 rounded-2xl border border-slate-800 p-12 text-center">
      ...
      <div className="space-y-4">
        {reports.map((r) => {
            ...
            return (<div key={r.id} className="bg-slate-900 rounded-xl border border-slate-800 p-5 hover:border-slate-600 transition-colors">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {r.author_name && (<p className="text-xs text-slate-500 mt-2">by {r.author_name}</p>)}
                </div>
