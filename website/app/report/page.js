...
export default function ReportPage() {
    ...
    return (<div className="container mx-auto px-4 py-10 max-w-4xl">
      ...
      {loading && (<div className="text-theme-fg-muted text-center py-16 animate-pulse text-sm">Loading reports…</div>)}

      {!loading && error && (<div className="bg-theme-panel rounded-2xl border border-theme-border p-12 text-center space-y-4">
          <p className="text-theme-fg-muted text-base">No reports to display yet.</p>
          <p className="text-theme-fg-muted text-sm">{error}</p>
          <button onClick={load} className="mt-2 px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-sm font-semibold text-theme-fg transition-colors">
            Try Again
          </button>
        </div>)}

      {!loading && !error && reports.length === 0 && (<div className="bg-theme-panel rounded-2xl border border-theme-border p-12 text-center">
      ...
      <div className="space-y-4">
        {reports.map((r) => {
            ...
            return (<div key={r.id} className="bg-theme-panel rounded-xl border border-theme-border p-5 hover:border-slate-600 transition-colors">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {r.author_name && (<p className="text-xs text-theme-fg-muted mt-2">by {r.author_name}</p>)}
                </div>
                <div className="text-right shrink-0">
                  ...
                  {r.location?.coordinates && (<p className="text-[11px] text-slate-600 mt-1">
