...
export class AuthService {
    ...
    static async register(data) {
        ...
        const isVulgar = vulgarWords.some(word => nameLower.includes(word) ||
        ...
        const method = verificationMethod === 'otp' ? 'otp' : 'link';
        ...
        const result = await pool.query(`INSERT INTO users (email, password_hash, name, role, is_verified, verification_token, verification_token_expires, verification_otp, verification_method)
       VALUES ($1, $2, $3, $4, false, $5, $6, $7, $8)
       RETURNING id, email, name, role`, [email, hashedPassword, trimmedName, 'user', verificationToken, verificationExpires, verificationOtp, method]);
        const user = result.rows[0];
    ...
    static async verifyEmail(token) {
       ...
       WHERE id = $1`, [user.id]);
        const updatedResult = await pool.query('SELECT id, email, name, role, is_verified FROM users WHERE id = $1', [user.id]);
        const updatedUser = updatedResult.rows[0];
        const jwtToken = this.generateToken(updatedUser);
        return { user: updatedUser, token: jwtToken };
    }
    static async resendVerification(email, verificationMethod) {
        const result = await pool.query('SELECT * FROM users WHERE email = $1', [email]);
        const user = result.rows[0];
        ...
        if (user.verification_token_expires) {
            ...
            const durationMs = user.verification_method === 'otp' ? 15 * 60 * 1000 : 24 * 60 * 60 * 1000;
        ...
        const method = (verificationMethod === 'otp' || (!verificationMethod && user.verification_method === 'otp')) ? 'otp' : 'link';
        let verificationToken = null;
        let verificationOtp = null;
        let verificationExpires;
        if (method === 'otp') {
       ...
       WHERE id = $5`, [verificationToken, verificationExpires, verificationOtp, method, user.id]);
        if (method === 'otp') {
            EmailService.sendVerificationOtpEmail(email, verificationOtp).catch((err) => {
    ...
    static async requestPasswordReset(email) {
        ...
        await pool.query(`UPDATE users SET reset_token = $1, reset_token_expires = $2 WHERE id = $3`, [resetOtp, resetExpires, user.id]);
    ...
    static async toggle2fa(userId, enable) {
    ...
    static async changePassword(userId, oldPassword, newPassword, otpToken) {
        if (!newPassword || newPassword.length < 8) {
    ...
    static generateToken(user) {
        return jwt.sign({ id: user.id, email: user.email, role: user.role, two_factor_enabled: user.two_factor_enabled }, JWT_SECRET, { expiresIn: '24h' });
    }
}
