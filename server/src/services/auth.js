...
export class AuthService {
    ...
    static async register(data) {
        ...
        const verificationExpires = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24 hours
        const result = await pool.query(`INSERT INTO users (email, password_hash, name, role, is_verified, verification_token, verification_token_expires)
       VALUES ($1, $2, $3, $4, false, $5, $6)
       RETURNING id, email, name, role`, [email, hashedPassword, trimmedName, 'user', verificationToken, verificationExpires]);
        const user = result.rows[0];
    ...
    static async resendVerification(email) {
       ...
       WHERE id = $3`, [verificationToken, verificationExpires, user.id]);
